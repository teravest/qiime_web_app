...
def unassigned_kits(opts, cursor, existing_kit_ids):
    ...
    if ',' in opts.swabs_per_kit:
        n_swabs = map(int, opts.swabs_per_kit.split(','))
        n_kits = map(int, opts.number_of_kits.split(','))
    else:
        n_swabs = [int(opts.swabs_per_kit)]
        n_kits = [int(opts.number_of_kits)]

    for kit_count,swab_count in zip(n_kits, n_swabs):
